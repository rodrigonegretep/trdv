...
class CommunityControllerProfiles extends CommunityController
{
	...
	public function saveOrder()
	{
		...
		// Get the ID in the correct location
 		$id			= $jinput->post->get('cid', array(), 'array'); //JRequest::getVar( 'cid', array(), 'post', 'array' );
		$db			= JFactory::getDBO();
	...
	public function ajaxSaveField( $id , $data )
	{
		...
		if( $isValid )
		{
			...
			if($id != 0)
			{
				...
				// reload the page instead update the particular row, so user won't confuse
				$response->addScriptCall('window.location.href = "' . JURI::base() . 'index.php?option=com_community&view=profiles";');
			}
			else
			{
...
<table cellspacing="0" border="0" width="100%">
	<tbody>
		...
		<tr>
			<td class="key"><span class="js-tooltip" title="<?php echo JText::_('COM_COMMUNITY_PROFILE_REQUIRED_TIPS');?> "><?php echo JText::_('COM_COMMUNITY_REQUIRED');?></span></td>
		...
		<tr class="fieldToolTip" <?php echo $row->type == 'checkbox' ? 'style="display:none;"' : 'style="display:table-row;"' ; ?>>
			<td valign="top" class="key"><span class="js-tooltip" title="<?php echo JText::_('COM_COMMUNITY_PROFILE_TOOLTIP_TIPS');?> "><?php echo JText::_('COM_COMMUNITY_TOOLTIP');?></span></td>
			<td colspan="4">
...
<?php
		...
		$response->addScriptCall('(function($) { $$(".js-tooltip, .hasTooltip").each(function (e) {e.hide = null;});})(MooTools);');
		$response->addScriptCall( 'cWindowActions' , $buttons );
		return $response->sendResponse();
	}
