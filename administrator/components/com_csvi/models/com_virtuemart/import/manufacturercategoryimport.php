 ...
 * @version 	$Id: manufacturercategoryimport.php 2357 2013-03-04 07:31:45Z RolandD $
 */
...
class CsviModelManufacturercategoryimport extends CsviModelImportfile {
	...
	public function getProcessRecord() {
		...
		if (!isset($this->virtuemart_manufacturercategories_id)) $this->_getManufacturerCategoryId();

		// Set the modified date as we are modifying the product
		if (!isset($this->modified_on)) {
			$this->_manufacturer_categories->modified_on = $this->date->toSql();
		...
		if (empty($this->virtuemart_manufacturercategories_id)) {
			$this->_manufacturer_categories->created_on = $this->date->toSql();
		...
		if ($this->virtuemart_manufacturercategories_id && $this->mf_category_delete == "Y") {
			if ($this->_manufacturer_categories->delete($this->virtuemart_manufacturercategories_id)) {
		...
		else if (!$this->virtuemart_manufacturercategories_id && $template->get('ignore_non_exist', 'general')) {
