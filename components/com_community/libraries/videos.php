...
class CVideos extends JObject
{
	...
	public function runConvert()
	{
		...
		$model		= CFactory::getModel('videos');
                ...
                $tableBind = false;
                $limitSetting = true;
                $useCache = false; /* NO caching here */
		$videos		= $model->getPendingVideos($filters,$tableBind,$limitSetting,$useCache);
...
// 		$data[]	= '-metadatacreator:' . 'Jomsocial.com';
	...
	static public function getActivityTitleHTML($act)
	{
		...
		$html = '<a class="cStream-Author" href="'.CUrlHelper::userLink($act->actor).'">'.$actor->getDisplayName().'</a>';;
		$param 	 = new JRegistry( $act->params );

		// If we're using new stream style or has empty content
		if($param->get('style') == COMMUNITY_STREAM_STYLE || empty($act->content) ){
			$html .= JText::_('COM_COMMUNITY_ACTIVITY_VIDEO_SHARE_TITLE');
		}
	...
	static public function getVideoReturnUrlFromRequest($videoType='default')
	{
		...
		$creator_type	= $jinput->get('creatortype' , VIDEO_USER_TYPE, 'NONE'); //JRequest::getVar( 'creatortype' , VIDEO_USER_TYPE );
		$groupId		= JRequest::getInt( 'groupid' , 0 );
		$my				= JFactory::getUser();

		// we use this if redirect url is defined
		$redirectUrl	= $jinput->post->get('redirectUrl' , '', 'STRING'); //JRequest::getVar( 'redirectUrl' , '' , 'POST' );
