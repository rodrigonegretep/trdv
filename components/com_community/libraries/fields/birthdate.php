...
class CFieldsBirthdate extends CFieldsDate
{
        ...
        public function __construct($fieldId = null) {
            ...
            if(is_object($this->params)){
                ...
                $minRanger = $this->params->get('minrange', 1);
                $maxRanger = $this->params->get('maxrange', 100);
                $this->_yearRanger = $this->_getYearRange($maxRanger) . ':' . $this->_getYearRange($minRanger);
            }
        }
        ...
        public function getFieldData( $field )
	{
		...
		else
		{
			...
			$format = $params->get('date_format','Y-m-d');
	...
	public function isValid( $value , $required )
	{
         ...
         * @return int
         */
        private function _getYearRange($val){
            $rangeYear = 0;
            /* Valid datetime */
            if( strtotime($val)!= false && strtotime($val) != -1 ){
                $tmpDate = new DateTime($val);
                $rangeYear = (int)$tmpDate->format('Y');
            }else{
                $rangeYear = (int)(date('Y', time()) - $val);
            }
            return $rangeYear;
        }
}