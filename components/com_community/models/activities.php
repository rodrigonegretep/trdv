...
class CommunityModelActivities extends JCCModel {
    ...
    public function countActivities($userid = '', $friends = '', $afterDate = null, $maxEntries = 0, $respectPrivacy = true, $actidRange = null, $displayArchived = true, $actid = null, $groupid = null, $eventid = null) {
        $db = $this->getDBO();
     ...
     */
    public function getActivities($userid = '', $friends = '', $afterDate = null, $maxEntries = 20, $respectPrivacy = true, $actidRange = null, $displayArchived = true, $actid = null, $groupid = null, $eventid = null) {
    ...
    public function getAppActivities($options) {
        ...
        $sql = 'SELECT a.* , (DAY( ' . $db->Quote($date->toSql(true)) . ' ) - DAY( DATE_ADD(a.' . $db->quoteName('created') . ',INTERVAL ' . $date->getOffset() . ' HOUR ) )) as ' . $db->Quote('_daydiff')
                ...
                . ' LIMIT ' . (int) max(0, $limit); /* do never use $limit without (int) to prevent SQL Injection */
        $db->setQuery($sql);
