...
class CommunityEventsController extends CommunityBaseController
{
	...
	public function ajaxWarnEventDeletion($eventId)
	{
		...
		$actions .= '<input type="button" class="btn btn-primary pull-right" onclick="jax.call(\'community\', \'events,ajaxDeleteEvent\', \'' . $eventId . '\', 1,' . $selected . ');" value="' . JText::_('COM_COMMUNITY_DELETE') . '"/>';
	...
	public function ajaxRequestInvite($eventId, $redirectUrl)
	{
		...
		$actions = '<input onclick="cWindowHide();" type="submit" value="' . JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . '" class="btn" name="Submit"/>';

		if (!$isMember) {
			$actions = '<form name="jsform-events-ajaxrequestinvite" method="post" class="reset-gap" action="' . CRoute::_('index.php?option=com_community&view=events&task=requestInvite') . '">';
			$actions .= '<input type="hidden" value="' . $eventId . '" name="eventid" />';
			$actions .= '<input onclick="cWindowHide();" type="button" value="' . JText::_('COM_COMMUNITY_NO_BUTTON') . '" class="btn" name="Submit" />';
	...
	<?php
		...
		$actions = '<form class="reset-gap" name="jsform-events-ajaxignoreevent" method="post" action="' . CRoute::_('index.php?option=com_community&view=events&task=ignore') . '">';
	...
	public function ajaxDeleteEvent($eventId, $step = 1, $action = '')
	{
		...
		if (!$handler->manageable() && $step == 1) {
			...
			$actions = '<input type="button" class="btn" onclick="cWindowHide();" value="' . JText::_('COM_COMMUNITY_CANCEL') . '"/>';
		...
		switch ($step) {
			...
			case 4:
				...
				$actions = '<input type="button" class="btn" id="eventDeleteDone" onclick="cWindowHide(); window.location=\'' . $redirectURL . '\';" value="' . JText::_('COM_COMMUNITY_DONE_BUTTON') . '"/>';
    ...
    <button class="btn btn-primary pull-right" onclick="joms.events.removeGuest(<?php echo $userid; ?>, <?php echo $eventid; ?>)"
            name="yes">
	...
	public function ajaxConfirmBlockGuest($userid, $eventid)
	{
		...
		$actions .= '<button class="btn btn-primary pull-right" onclick="joms.events.blockGuest(' . $userid . ',' . $eventid . ');">' . JText::_('COM_COMMUNITY_YES') . '</button>';
	...
	public function ajaxConfirmUnblockGuest($userid, $eventid)
	{
		...
		$actions = '<button class="btn" onclick="cWindowHide();">' . JText::_('COM_COMMUNITY_NO') . '</button>';
		$actions .= '<button  class="btn btn-primary pull-right" onclick="joms.events.unblockGuest(' . $userid . ',' . $eventid . ');">' . JText::_('COM_COMMUNITY_YES') . '</button>';
			...
			if (!$model->isExists(FEATURED_EVENTS, $eventId)) {
				...
				$my = CFactory::getUser();
                                ...
                                if($featured->add($eventId, $my->id)===true){
                                    ...
                                    $html = JText::sprintf('COM_COMMUNITY_EVENT_LIMIT_REACHED_FEATURED', $table->title, $limit);
                                }
				
			} else {
				$html = JText::_('COM_COMMUNITY_EVENT_ALREADY_FEATURED');
			}
		} else {
	...
	public function ajaxShowRepeatOption()
	{
		...
		$action = '<button class="btn" onclick="joms.jQuery(\'#repeataction\').val(' . $selected . ');joms.jQuery(\'#createEvent\').submit()">' . JText::_('COM_COMMUNITY_SAVE_BUTTON') . '</button><br>';
