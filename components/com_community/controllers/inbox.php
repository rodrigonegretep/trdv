...
class CommunityInboxController extends CommunityBaseController
{
	...
	public function write()
	{
		...
		$data->to			= $jinput->post->get('friends', array(), 'array'); //JRequest::getVar ( 'friends', array(), 'POST','array' );
		$data->subject		= $jinput->post->get('subject', '', 'STRING') ; //JRequest::getVar ( 'subject', '', 'POST' );
		$data->subject		= $inputFilter->clean($data->subject);
		$data->body			= $jinput->post->get('body', '', 'STRING'); //JRequest::getVar ( 'body', '', 'POST' );
	...
	public function remove() {
		...
		$msgId = $jinput->get->get('msgid', '', 'INT'); //JRequest::getVar ( 'msgid', '', 'GET' );
		$my = JFactory::getUser ();
		$view =  $this->getView ( 'inbox' );
		$model =  $this->getModel ( 'inbox' );
	...
	public function ajaxRemoveFullMessages($msgIds){
		...
		$actions    =	'<button class="btn" onclick="cWindowHide();">' . JText::_('COM_COMMUNITY_CLOSE_BUTTON') . '</button>';

		$objResponse->addAssign('cwin_logo', 'innerHTML', $header);
		$objResponse->addScriptCall('cWindowAddContent', $message, $actions);
		$objResponse->addScriptCall ( 'joms.jQuery(\'#checkall\').prop(\'checked\', false)' );
	...
	public function ajaxCompose($id) {
		...
		if( ($dateNow->toUnix() - $lastSent->toUnix()) < $config->get( 'floodLimit' ) && !COwnerHelper::isCommunityAdmin() ){
			...
			$actions = '<button class="btn" onclick="cWindowHide();" name="cancel">' . JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . '</button>';
		...
		$actions .= '<button class="btn btn-primary pull-right" onclick="javascript:return joms.messaging.send();" name="send">'. JText::_('COM_COMMUNITY_SEND_BUTTON') .'</button>&nbsp;';
	...
	public function ajaxSend($postVars)
	{
		...
		{
			$html = '<dl id="system-message"><dt class="alert">' . JText::_('COM_COMMUNITY_NOTICE') . '</dt><dd class="notice message"><ul><li>';
		...
		{
			$errMsg = '<div class="alert">'.JText::_('COM_COMMUNITY_INBOX_SUBJECT_MISSING').'</div>';
			$doCont = false;
			$resizeH += 35;
		}
		...
		{
			$errMsg .= '<div class="alert">'.JText::_('COM_COMMUNITY_INBOX_MESSAGE_MISSING').'</div>';
			$doCont = false;
			$resizeH += 35;
		}
	...
	public function ajaxDeleteMessages($task,$empty="")
	{
		...
		if(!$empty){
			...
			$actions   .=	'<button class="btn btn-primary pull-right" onclick="joms.messaging.deleteMarked(\'' . $task . '\');">' . JText::_('COM_COMMUNITY_YES') . '</button>';
			
		}
		else{
			$message    =	JText::_('COM_COMMUNITY_INBOX_REMOVE_CONFIRM_NO_MESSAGE_CHECKED');
