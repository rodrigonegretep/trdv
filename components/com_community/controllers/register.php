...
class CommunityRegisterController extends CommunityBaseController
{
	...
	public function register_save()
	{
		...
		$formToken = $jinput->request->get('authkey', '', 'STRING'); //JRequest::getVar('authkey', '', 'REQUEST');
		...
		if ($config->get('recaptcha') == 1 && ! empty($public) && ! empty($private))
		{
			...
			$response   = CRecaptchaHelper::recaptcha_check_answer( $private , $ipAddress , $_POST['recaptcha_challenge_field'] , $_POST['recaptcha_response_field'] );
	...
	public function registerProfileType()
	{
		...
		{
			$type = $jinput->get('profileType', 0, 'INT'); //JRequest::getVar('profileType', 0);
	...
	public function registerUpdateProfile()
	{
		...
		$profileType    = $jinput->post->get('profileType', 0, 'NONE'); //JRequest::getVar('profileType', 0, 'POST');
		$multiprofile   = JTable::getInstance('MultiProfile', 'CTable');
		$multiprofile->load( $profileType);
	...
	public function sendEmail($type, $user, $password = null, $requireApproval = false)
	{
		...
		$baseUrl        = JUri::base();
		$activationURL  = $baseUrl.'index.php?option='.COM_USER_NAME.'&task='.COM_USER_TAKS_ACTIVATE.'&'.ACTIVATION_KEYNAME.'='.$user->get('activation');
		...
		switch ($type)
		{
			...
			case 'registration_complete' :
				...
				else
				{
					...
					{
						$message = JText::sprintf('COM_COMMUNITY_EMAIL_REGISTRATION_COMPLETED', $name, $sitename, $siteURL);
					}
				}
    ...
    public function activationResend() {
        ...
        $jsEmail = $jinput->request->get('jsemail', '', 'STRING'); //JRequest::getVar('jsemail', '', 'REQUEST');
