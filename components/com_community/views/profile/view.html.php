...
class CommunityViewProfile extends CommunityView {
    ...
    public function profile($data) {
        ...
        /* set head meta */
        CHeadHelper::addOpengraph('og:type', 'profile');
        CHeadHelper::setTitle(JText::sprintf('COM_COMMUNITY_USER_PROFILE_PAGE_TITLE', $user->getDisplayName()));
        CHeadHelper::setDescription($user->_status);
        CHeadHelper::addOpengraph('og:image', $user->getAvatar(), true);
		CHeadHelper::addOpengraph('og:image', $user->getCover(), true);
    ...
    public function modProfileUserinfo() {
        ...
        $profile->status = $user->getStatus();
        $profile->defaultCover = (empty($user->_cover)) ? true : false;
        $profile->cover = $user->getCover();
        $profile->coverPostion = $params->get('coverPosition',0);
