	...
	public function album()
	{
		...
		//$document->setTitle( JText::sprintf( 'COM_COMMUNITY_PHOTOS_USER_PHOTOS_TITLE' ,  $handler->getCreatorName() ) .' - '. $album->name );
                ...
                if (
                    ...
                    CHeadHelper::setTitle( JText::sprintf( 'COM_COMMUNITY_PHOTOS_USER_PHOTOS_TITLE' ,  $handler->getCreatorName() ));
                }else {
                    CHeadHelper::setTitle( JText::sprintf( 'COM_COMMUNITY_PHOTOS_USER_PHOTOS_TITLE' ,  $handler->getCreatorName() ) .' - '. $album->name );
                }
                CHeadHelper::setDescription(CStringHelper::escape($album->getDescription()));
		...
		for( $i = 0; $i < count( $photos ); $i++ )
		{
			...
			$photo->link = $handler->getPhotoURI($photo->id , $photo->albumid);
                        CHeadHelper::addOpengraph('og:image',rtrim(JUri::root(),'/') . '/' . $photo->image,true);
		}
		...
		$wallContent	= CWallLibrary::getWallContents( 'albums' , $album->id , ( COwnerHelper::isCommunityAdmin() || ($my->id == $album->creator && ($my->id != 0)) ) , 10 , 0, 'wall.content', 'photos,album');
...
//		}
	...
	public function photo()
	{
        ...
        /* set head meta */
        CHeadHelper::setTitle( $album->name . ' - ' . $photo->caption );
        CHeadHelper::addOpengraph('og:image',rtrim(JUri::root(),'/') . '/' . $photo->image,true);
        /**
         * @todo add og:image:type/width/height
         */
	...
	public function setSubmenus()
	{
		...
		switch( $task )
		{
			...
			default:
				...
				if( $task == 'album' && ($my->id == $album->creator || COwnerHelper::isCommunityAdmin()) )
				{
