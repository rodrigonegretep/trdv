...
class CommunityViewEvents extends CommunityView {
    ...
    public function viewevent() {
        ...
        /* set head meta */
        CHeadHelper::setTitle(JText::sprintf('COM_COMMUNITY_EVENT_PAGE_TITLE', $event->title));
        ...
        if (!isset($event->params)) $event->params = '';
        $params = new CParameter($event->params);

        $event->coverPostion = $params->get('coverPosition',0);
        $event->defaultCover = (empty($event->cover)) ? true : false;
        ...
        /* Opengraph */
        CHeadHelper::addOpengraph('og:image', $event->getAvatar('avatar'), true);
        CHeadHelper::addOpengraph('og:image', $event->getCover(), true);
