...
<div class="js-focus">
	<div class="js-focus-cover">
		<img id='<?php echo $event->id; ?>' class="focusbox-image cover-image" src="<?php echo $event->getCover(); ?>" alt="cover photo" style="top:<?php echo $event->coverPostion; ?>">
		...
		<?php if( $isMine || COwnerHelper::isCommunityAdmin() || $isAdmin ){?>
			...
			<div class="js-focus-change-cover" data-cover-type="cover">
				 	<?php if(!$event->defaultCover){?>
				 		<a href="javascript:void(0)" class="btn" data-cover-context="event" onclick="joms.cover.reposition(this, <?php echo $event->id; ?>,'<?php echo JText::_("COM_COMMUNITY_CANCEL_BUTTON")?>','<?php echo JText::_("COM_COMMUNITY_SAVE_BUTTON")?>');"><i class="js-icon-cog"></i><?php echo JText::_('COM_COMMUNITY_REPOSITION_COVER')?></a>
				 	<?php }?>
				 	<a href="javascript:void(0)" class="btn ml-6" data-cover-context="event" onclick="joms.cover.select(this, <?php echo $event->id; ?>);" ><i class="js-icon-picture"></i><?php echo JText::_('COM_COMMUNITY_CHANGE_COVER'); ?></a>
				</div>
		<?php }?>
		      ...
		      <div class="nav-collapse collapse js-collapse">
		        <ul class="nav">
					...
					<?php if($isLikeEnabled){?>
						<li id='like-events-<?php echo $event->id; ?>'><a href="javascript:void(0);" onclick="<?php echo ($isUserLiked) > 0 ? 'joms.like.newDislike(this)' : 'joms.like.newLike(this)' ?>;" class="<?php if($isUserLiked > 0){ ?> js-focus-like <?php }?>"><i class="js-icon-thumbs-up"></i><span><?php echo $totalLikes ?></span> <?php echo JText::_('COM_COMMUNITY_LIKE')?></a></li>
					<?php }?>
					<li><span><i class="js-icon-eye"></i><?php echo ($event->hits > 1) ? JText::sprintf('COM_COMMUNITY_VIDEOS_HITS_COUNT_MANY',$event->hits) : JText::sprintf('COM_COMMUNITY_VIDEOS_HITS_COUNT',$event->hits); ?></span></li>
					<?php if($config->get('enablereporting') == 1 && ( $config->get('enableguestreporting') !=1 && $my->id !=0)){?>
						<li><a href="javascript:void(0)" onclick="joms.report.emptyMessage = '<?php echo JText::_('COM_COMMUNITY_REPORT_MESSAGE_CANNOT_BE_EMPTY'); ?>';joms.report.showWindow('events,reportEvent','[<?php echo  $event->id  ;?>]');" ><i class="js-icon-attention"></i><?php echo JText::_('COM_COMMUNITY_EVENTS_REPORT'); ?></a></li>
			      	<?php }?>
			      </ul>
