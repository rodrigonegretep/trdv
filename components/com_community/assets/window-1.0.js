...
{
	//Limit cWindow size
    var maxH = joms.jQuery(window).height() * 0.85;
    var minH = joms.jQuery(window).height() * 0.13;
	if (h > maxH) h = maxH;
    if (h < minH) {
        if(cWindowHelper.contentOuterHeight < maxH)
        {
            h = cWindowHelper.contentOuterHeight;
        }else{
            h = maxH;
        }
    };
...
function cWindowActions(action)
{
	...
	if (action)
	{
		joms.jQuery('<div id="cWindowAction" class="modal-footer">')
				.html(action)
				.css('bottom', '-30px')
				.appendTo('#cWindowContentOuter')
				.animate({bottom: '0px'});
