...
joms.extend({
	...
	activities: {
		...
		append: function( html ){
			// Filter only the list
			var data = joms.jQuery(html).find('ul.cStreamList li');

			joms.jQuery( '#activity-more,#activity-exclusions' ).remove();
			joms.jQuery( '#activity-stream-container ul.cStreamList' ).append( html );
		...
		/**
		 * Load more stream post at the bottom of the stream list
		 */
		more: function(){
			// List object
			var stream = joms.jQuery('.cMain > #activity-stream-container ul.cStreamList');

                        //Get stream of group & event
                        if(stream.length <= 0)
                            stream = joms.jQuery('#activity-stream-container ul.cStreamList');
			...
			jax.call( 'community' , 'activities,ajaxGetOlderActivities' , exclusions , filter, filterId);
		},
		...
		{
                        //Get container
                        container = joms.jQuery('.cMain > #activity-stream-container');

                        //Get group & event container
                        if(container.length <= 0)
                            container = joms.jQuery('#activity-stream-container');

                        // We received full <ul> list, just grab the li part
			var dataList = joms.jQuery(html).find('li');

			// Insert liat at the top
			container.find('li[data-streamid]').last().after(dataList);

			container.find( '#activity-more .more-activity-text' ).show();
			container.find( '#activity-more .loading' ).hide();
		},
		...
		{
                        if(joms.jQuery('.joms-latest-activities-container').data('actid') > -1){
                            joms.jQuery('.joms-latest-activities-container').css('display', 'none');
                            return;
                        }
	...
	invitation:{
		...
		selectMember:function( element ){
			...
			joms.jQuery('#community-invited-list '+ element).remove();
	...
	notifications: {
		...
		showRequest: function(){
			...
			var height = joms.jQuery('#cWindowContent').height() + 40;
			cWindowResize (height);
		},
		showInbox: function(){
	...
	groups: {
		...
		editBulletin: function(){
			...
			if( joms.jQuery('#bulletin-edit-data').css('display') == 'none' )
			{
				...
				joms.jQuery('.cPageStory-Author,.cPageStory-Content').hide();
			}
			else
			{
				joms.jQuery('#bulletin-edit-data').hide();
				joms.jQuery('.cPageStory-Author,.cPageStory-Content').show();
			}
		...
		unpublish: function( groupid ){
			var ajaxCall = 'jax.call("community", "groups,ajaxShowUnpublishGroup", "' + groupid + '");';
			cWindowShow(ajaxCall, '', 450, 100);
		},
	...
	photos: {
		multiUpload: {
			...
			_init: function (groupid, uploadMsg)
			{
				...
				if ((pluploader.pluploadQueue().runtime == 'html4') || (joms.jQuery.browser.msie && (parseInt(joms.jQuery.browser.version) <= 7)) || (joms.jQuery.browser.mozilla && (joms.jQuery.browser.version.slice(0,3))==1.9))
				{
				...
				{
					joms.jQuery('div.plupload.html5').css('top',230);
				}
			},
			log: function ()
			{
			...
			assignUploadUrl: function( albumId )
			{
				...
				if (urlDefault.match(/&+/))
				{
					urlDefault = urlDefault + '&albumid='+albumId + '&batchcount='+uploader.files.length;
				}
				else
				{
					urlDefault = urlDefault + '?albumid='+albumId + '&batchcount='+uploader.files.length;
		...
		ajaxUpload : function(type,id, isCustom){
					...
					success:function(data,status)
					{
						...
						else
						{
							...
							joms.jQuery('#large-avatar-pic').load(function (){
                                                            var resizeW =  Math.round((64 / 160) * joms.jQuery('#large-avatar-pic').width());
                                                            var resizeH = Math.round((64 / 160) * joms.jQuery('#large-avatar-pic').height());
							...
							{
								case 'event':
									oldAvatar = joms.jQuery('.js-focus-avatar img').prop('src');
                                                                        var eventAvatar = url.replace('event-','');

									joms.jQuery('.js-focus-avatar img').prop('src',eventAvatar);
								...
								case 'group':
									oldAvatar = joms.jQuery('.js-focus-avatar img').prop('src');
                                                                        var groupAvatar = url.replace('group-','');

									joms.jQuery('.js-focus-avatar img').prop('src',groupAvatar);
								...
								case 'profile':
									...
									if (data.info.length != 0)
									{
											...
											var profileAvatar = thumbUrl.replace('thumb_','');
											...
											joms.jQuery('.js-focus-avatar img').prop('src',profileAvatar);
									}
									if(oldAvatar.search('assets/user-') > -1){
											jax.call('community', 'photos,ajaxUploadAvatar',type,id);
									}
									break;
							}
			...
			return false;
                ...
                loadImgSelect: function(){
                    joms.jQuery('#large-avatar-pic').unbind('load');
                    joms.jQuery('#large-avatar-pic').load(function (){
		...
		ajaxImgSelect : function(){
		    var imgH = joms.jQuery('#large-avatar-pic').height();
			var imgW = 160;
		...
		previewThumb : function (img,selection){
			...
			var scaleY = 64 / (selection.height || 1);
                       joms.jQuery('#thumb-hold  >img').css(
                              {
                                      width: Math.round(scaleX * joms.jQuery('#large-avatar-pic').width()) + 'px',
                                      height: Math.round(scaleY * joms.jQuery('#large-avatar-pic').height()) + 'px',
	...
	messaging: {
		...
		confirmDeleteMarked: function(task){
		    ...
		    if(msgIds.toString()!=''){
		    	...
		    	var ajaxCall = 'jax.call("community", "inbox,ajaxDeleteMessages", "'+task+'","empty")'
		    }
	...
	walls: {
		...
		insert: function( html ){
			...
			if(joms.walls.insertOrder == 'prepend'){
				...
				// append
				joms.jQuery('#wall-containter .wallComments:last').after(html);
			}
		},
		...
		edit: function( id , permissionFunc ){
			...
			if( joms.jQuery('#wall-edit-' + id ).val() != null )
			{
				...
				joms.jQuery('#wall-edit-container-'+ id).children().remove();
			}
			else
			{
	...
	miniwall:{
		...
		add: function(id, host) {
                        ...
                        if(joms.miniwall.type(host) == 'cModule-ActivityStream')
                        {
                            cmtBlock = joms.jQuery("#cModule-ActivityStream").find("#wall-cmt-"+ id);
                        }
                        else
                        {
                            cmtBlock = joms.jQuery(".cMain").find("#wall-cmt-"+ id);
                        }

                        var cmt = cmtBlock.find('textarea').val();
                        cmt = joms.jQuery.trim(cmt);

                        if(cmt.length > 0)
                        {
                                cmtBlock.find('.wall-coc-form-action.add').attr('disabled', true);
                                cmtBlock.find('.wall-coc-errors').hide();

                                jax.loadingFunction = function() {
                                        cmtBlock.find('textarea').attr('disabled', true);
                                        cmtBlock.find('.wall-coc-form-actions').append('<em class="wall-cmt-loading">Posting...</em>');
                                        jax.loadingFunction = function() {}				// clear the callbacks after each call
                                };

                                jax.doneLoadingFunction = function() {
                                        cmtBlock.find('.wall-coc-form-actions').find('em').remove();
                                        cmtBlock.find('textarea').attr('disabled', false).val('');
                                        cmtBlock.find('.wall-coc-form-action.add').attr('disabled', false);

                                        // update comment count
                                        cmtCountObj = cmtBlock.parent().parent().find('.wall-cmt-count');
		...
		show: function(id, host){
                        ...
                        if(joms.miniwall.type(host) == 'cModule-ActivityStream')
                        {
                            targetBlock = joms.jQuery("#cModule-ActivityStream").find('li[data-streamid='+ id + '] [data-formblock]');
                            replyBlock = joms.jQuery("#cModule-ActivityStream").find('li[data-streamid='+ id + '] [data-replyblock]');
                        }
                        else
                        {
                            targetBlock = joms.jQuery(".cMain").find('li[data-streamid='+ id + '] [data-formblock]');
                            replyBlock = joms.jQuery(".cMain").find('li[data-streamid='+ id + '] [data-replyblock]');
                        }

                        targetBlock.show();
                        targetBlock.find("button").attr('onclick',"joms.miniwall.add('" + id + "', this);return false;");
                        replyBlock.hide();

                        // @todo: should only autogrow once.
			var textarea = targetBlock.find('textarea');
			if( !textarea.data('autogrow') )
			{
                ...
                type: function(host)
                {
                    var tracker = joms.jQuery(host);
                    //Make sure we won't jump to an infinite loop
                    var limitor = 50;
                    var htype = '';
                    while(tracker.prop("tagName") != undefined && tracker.prop("tagName") != joms.jQuery("body").prop("tagName") && limitor > 0)
                    {
                        if(htype == '' && ( tracker.attr("id") == 'cModule-ActivityStream' || tracker.attr("class") == 'cMain' ))
                        {
                            htype = (tracker.attr("id") != undefined)?tracker.attr("id"):tracker.attr("class");
                        }
	...
	comments:{
		...
		edit: function(id,parentid, host){
                          //Isn't an event
                        if(typeof host == 'undefined')return;
                        ...
                        {
                            targetBlock = joms.jQuery(".cMain");
                        }

			targetBlock.find('div[data-commentid='+id+']').hide();
			var temp = document.createElement("div");
			var oldValue 	= targetBlock.find('div[data-commentid='+id+'] .cStream-Content span.comment').html();

			targetBlock.find('li[data-streamid='+ parentid + '] [data-formblock]').show();
			targetBlock.find('li[data-streamid='+ parentid + '] [data-replyblock]').hide();

			// @todo: should only autogrow once.
			var textarea = targetBlock.find('li[data-streamid='+ parentid+'] .cStream-Form').find('textarea');
			temp.innerHTML = oldValue;
			var sanitized = temp.textContent || temp.innerText;
			textarea.val(sanitized);

			targetBlock.find('li[data-streamid='+ parentid + '] button').attr('onClick','joms.comments.saveEdit('+id+','+parentid+', this);return false;');
			targetBlock.find('li[data-streamid='+ parentid + '] .cStream-FormCancel').attr('onClick','joms.comments.cancelEdit('+id+','+parentid+', this);return false;');
		...
		cancelEdit: function(id,parentid){
			joms.jQuery('div[data-commentid='+id+']').show();
			joms.jQuery('li[data-streamid='+ parentid + '] .cStream-FormCancel').attr('onClick','joms.miniwall.cancel('+parentid+');return false;');
			joms.jQuery('li[data-streamid='+ parentid + '] button').attr('onClick','joms.miniwall.add('+parentid+');return false;');
			joms.miniwall.cancel(parentid);

		},
		saveEdit: function(id,parentid, host){
                        var targetBlock = null;
			...
			var value = textarea.val();
			 ...
			 jax.call('community','system,ajaxeditComment',  id , value , parentid );
		}
	},
	utils: {
		...
		textAreaWidth: function(target) {
			with (joms.jQuery(target))
			{
				...
				css('width', parseInt(parent().width())
	...
	videos: {
		...
		deleteVideo: function(videoId, currentTask, userId){
			var ajaxCall = "jax.call('community' , 'videos,ajaxRemoveVideo', '" + videoId + "','" + currentTask + "','" + userId + "');";
	...
	users: {
       ...
       removeCover: function(userId){
       	var ajaxCall = "jax.call('community', 'profile,ajaxRemoveCover', '" + userId + "');";
		cWindowShow(ajaxCall, '', 450, 100);
       }
	},
	...
	privacy: {
		init: function(){
			...
			joms.jQuery('.js_dropDownChild').live('click touchstart',function(e) {
				e.preventDefault();
			...
			joms.jQuery('.js_dropDownMaster dt').live('click touchstart', function(e) {
				e.preventDefault();
	...
	like : {
		init: function() {
			...
			joms.jQuery('a[href="#like"]').live('click', function(){
				var streamid = joms.jQuery(this).parents('li.stream').data('streamid');
				jax.call('community','system,ajaxStreamAddLike', streamid) ;
				return false;
			});
		...
		newLike: function(e){
			var like  = joms.jQuery(e).parent();
		    var data = this.extractData(like.attr('id'));

		    // Remove the onclick attribute value to avoid double processing
		    joms.jQuery(e).unbind('onclick');
		    var counter = joms.jQuery(joms.jQuery(e).children()[1]).html();

		    joms.ajax.call('system,ajaxLike', [data['element'], data['itemid']], {
				success: function(html)
				{
					counter = parseInt(counter)+1;
			    	joms.jQuery(e).addClass('js-focus-like');
					joms.jQuery(e).attr('onclick', 'joms.like.newDislike(this)');
					joms.jQuery(joms.jQuery(e).children()[1]).html(counter);
				}
		    });
		},
		newDislike: function(e){
			var like  = joms.jQuery(e).parent();
		    var data = this.extractData(like.attr('id'));
		    ...
		    joms.ajax.call('system,ajaxDislike', [data['element'], data['itemid']], {
				...
				{
					counter = parseInt(counter)-1;
				    joms.jQuery(e).removeClass('js-focus-like');
					joms.jQuery(e).attr('onclick', 'joms.like.newLike(this)');
					joms.jQuery(joms.jQuery(e).children()[1]).html(counter);
				}
		    });
		},
   ...
   file:{
       ...
       ajaxUploadFile: function(url,type,size)
       {
			...
			// General settings
			runtimes : 'gears,flash,silverlight,browserplushtml5,html4',
			url : url,
            ...
            joms.jQuery('input:file').parent().css('top','245px');
	   ...
	   {
		   	joms.jQuery('<iframe>', {src:url,id:'iframe'}).appendTo('body');
		   	//cWindowHide();
    ...
    cover:{
    	...
    	position  	: 0,
    	init: function() {
			joms.jQuery('.js-navbar-options, .js-navbar-icon').dropdown({
				toggle: false
			});

			/*joms.jQuery('.js-navbar-options, .js-navbar-icon').on('click',
				function(e){
					e.stopPropagation();
					joms.jQuery('.dropup.open').dropdown('toggle');
				}
			);*/

			// Navigation collapse
			var $collapseNav = joms.jQuery('.js-bar-collapse');

			$collapseNav.collapse({
				toggle: false
			});

			joms.jQuery('.js-bar-collapse-btn').on('click',
				function(e){
					e.preventDefault();
			...
			// Cover options
			var $collapseCover = joms.jQuery('.js-collapse');

			$collapseCover.collapse({
				toggle: false
			});
			...
			joms.jQuery('.js-collapse-btn').on('click',
				function(e){
					e.preventDefault();
					$collapseCover.collapse('toggle');
				}
			);

			// About me @TODO: use jQuery's toggle
			var $collapseAbout = joms.jQuery('.js-collapse-about');

			$collapseAbout.collapse({
				toggle: false
			});

			joms.jQuery('.js-collapse-about-btn').on('click',
				function(e){
					e.preventDefault();
    	...
    	select: function(e,id,updatetarget)
    	{
    		joms.cover.type = joms.jQuery(e).data('cover-context');
    		joms.cover.parentId = id;

    		var ajaxCall = 'jax.call("community","photos,ajaxChangeCover","'+joms.cover.type+'","'+joms.cover.parentId+'")';
			cWindowShow(ajaxCall, '', 600, 400);
    	},
    	openalbum: function(albumId,photocount)
    	{
    		jax.call("community","photos,ajaxGetPhotoList",albumId,photocount);
    	},
    	showlist: function(html)
    	{
    		joms.jQuery('#coverphotolist').html(html);
    		joms.jQuery('#coverphotolist').show();
			joms.jQuery('#coveralbumlist').hide(function(){/*cWindowAutoResize()*/});
    	},
    	setCover: function(e)
    	{
    		jax.call("community","photos,ajaxSetPhotoCover",joms.cover.type,e.id,joms.cover.parentId);
    	},
    	updateCover: function(path,cancelText,SaveText)
    	{
    		cWindowHide();
			...
			(
				{
					...
					success:function(data,status)
					{
						...
						{
							joms.cover.updateCover(data.path,data.cancelbutton,data.savebutton);
						}

						if(data.error)
						{
							if(!joms.jQuery('p#error_msg').is(':visible'))
							{
							    joms.jQuery('p#error_msg').show();
							}
							joms.jQuery('p#error_msg').html(data.error);
							joms.jQuery('p#error_msg').fadeOut(3000);
						}
					}
				}
			)
    	},
    	reposition: function(e,id,cancelText,SaveText){
    		joms.cover.type = joms.jQuery(e).data('cover-context');
    		joms.cover.parentId = id;
    		joms.jQuery('.js-focus-gradient').hide();
    		joms.jQuery('.js-focus-change-cover').hide();
    		var currentTop = joms.jQuery('img#'+id).position().top;
    		var maxHeight = joms.jQuery('.js-focus-cover').height() - joms.jQuery('img#'+id).height();
    		var button = '<div class="btn btn-primary" onClick="joms.cover.repositionCancel('+id+',\''+currentTop+'\')"><span>'+cancelText+'</span></div><div class="btn btn-primary ml-6" onClick="joms.cover.saveReposition()"><span>'+SaveText+'</span></div>';
    		joms.jQuery('.span4.text-right').html(button)
    		joms.jQuery('img#'+id).draggable({
    											axis: "y",
											    cursor: 'move',
											    ...
											    stop: function(event,ui){
											    	joms.cover.position = ui.position.top;
											    }
    								});
    	},
    	repositionCancel: function(id,currentTop){
    		joms.jQuery('.js-focus-gradient').show();
    		joms.jQuery('.js-focus-change-cover').css({'display':''});
    		joms.jQuery('.span4.text-right').html('');
    		joms.jQuery('img#'+id).css({'top':currentTop+'px'});
    		joms.jQuery('img#'+id).draggable( "destroy" );
    	},
    	saveReposition: function()
    	{
    		jax.call("community","photos,ajaxSetPhotoPhosition",joms.cover.type,joms.cover.parentId,joms.cover.position);
    		joms.jQuery('.js-focus-gradient').show();
    		joms.jQuery('.js-focus-change-cover').css({'display':''});
    		joms.jQuery('.span4.text-right').html('');
    		joms.jQuery('img#'+joms.cover.parentId).draggable( "destroy" );
    	}
   }
});
...
function update_counter(selector, count){
	...
	var currnum = parseInt(joms.jQuery(selector).html(), 10);
	count = parseInt(count, 10);
	if(currnum <= 1) {
		joms.jQuery(selector).css('display','none');
	} else{
		joms.jQuery(selector).html(currnum + count);
	}
}
...
joms.jQuery(document).ready(function () {
	...
	if(joms.jQuery.fn.dropdown !== undefined )
	{
		joms.cover.init();
	}
 ...
 * @link http://stackoverflow.com/questions/9784547/jquery-textarea-auto-grow-plugin-cross-browser-compatible
 */
